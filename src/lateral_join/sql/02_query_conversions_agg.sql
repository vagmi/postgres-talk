select sum(visited) visited_count,
       sum(registered) registered_count,
       sum(paid) paid_count
from (
    -- the visits query
  ...
  where visitor_id = visits.visitor_id  and
        ...
        created_at between visits.visited_time and (visits.visited_time + interval '3 months') -- 60 days
  order by created_at
  limit 1
  ...
  where visitor_id = visits.visitor_id and
        ...
        created_at between visits.visited_time and (visits.visited_time + interval '3 months')
  order by created_at
  limit 1
) payments on true;
